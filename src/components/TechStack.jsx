      ...
      <div className="max-w-screen-xl mx-auto px-4 text-center">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6">
          {technologies.map((tech, index) => (
            <div
              key={index}
              className="group p-6 bg-zinc-900/50 rounded-2xl border border-zinc-800/50 hover:border-indigo-500/50 hover:bg-zinc-800 transition-all duration-300 flex flex-col items-center justify-center gap-4"
            >
              <div className="p-3 rounded-xl bg-indigo-500/10 text-indigo-400 group-hover:text-indigo-300 group-hover:scale-110 transition-all duration-300">
                {/* 动态渲染大小为 32px 的图标 */}
                {React.cloneElement(getIcon(tech), { size: 32 })}
              </div>
              <h3 className="text-sm font-semibold text-zinc-400 group-hover:text-white transition-colors">{tech}</h3>
            </div>
          ))}
        </div>
      </div>
